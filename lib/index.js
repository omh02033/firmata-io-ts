...
export class Firmata extends Emitter {
    ...
    sendOneWireRead(pin, device, numBytesToRead, callback) {
        ...
        this[SYM_sendOneWireRequest](pin, ONEWIRE_READ_REQUEST_BIT, device, numBytesToRead, correlationId, undefined, null, `1-wire-read-reply-${correlationId}`, (data) => {
            clearTimeout(timeout);
            callback(null, data);
        });
    }
    ...
    sendOneWireWrite(pin, device, data) {
        this[SYM_sendOneWireRequest](pin, ONEWIRE_WRITE_REQUEST_BIT, device, undefined, undefined, undefined, Array.isArray(data) ? data : [data]);
    }
    /**
    ...
    sendOneWireDelay(pin, delay) {
        this[SYM_sendOneWireRequest](pin, ONEWIRE_DELAY_REQUEST_BIT, undefined, undefined, undefined, delay);
    }
    /**
    ...
    sendOneWireWriteAndRead(pin, device, data, numBytesToRead, callback) {
        ...
        this[SYM_sendOneWireRequest](pin, ONEWIRE_WRITE_REQUEST_BIT | ONEWIRE_READ_REQUEST_BIT, device, numBytesToRead, correlationId, undefined, Array.isArray(data) ? data : [data], `1-wire-read-reply-${correlationId}`, (data) => {
            clearTimeout(timeout);
            callback(null, data);
        });
    }
    ...
    accelStepperConfig(options) {
        const { deviceNum, invertPins, motorPin1, motorPin2, motorPin3, motorPin4, enablePin, stepSize = this.STEPPER.STEP_SIZE.WHOLE, type = this.STEPPER.TYPE.FOUR_WIRE, } = options;
        const data = [
            START_SYSEX,
            ACCELSTEPPER,
 ...
 */
export function writeToTransport(board, data) {
    board.pending++;
 ...
 */
export function i2cRequest(board, bytes) {
 ...
 */
export function encode32BitSignedInteger(data) {
 ...
 */
export function decode32BitSignedInteger(bytes) {
 ...
 * @returns An array of encoded numbers.
 */
export function encodeCustomFloat(input) {
 ...
 */
export function decodeCustomFloat(input) {
    const exponent = ((input[3] >> 2) & 0x0f) - 11;
    const sign = (input[3] >> 6) & 0x01;
    let result = input[0] | (input[1] << 7) | (input[2] << 14) | ((input[3] & 0x03) << 21);
    if (sign) {
        result *= -1;
    }
