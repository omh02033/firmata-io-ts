...
import OneWire from "./onewireutils";

interface I2CSettings {
	[key: string]: any;
}

// Program specifics
const i2cActive = new Map<Firmata, I2CSettings>();
...
const MIDI_RESPONSE: { [key: number]: (board: Board) => void } = {
	/**
	...
	[REPORT_VERSION](board: Board): void {
	...
	[ANALOG_MESSAGE](board: Board): void {
	...
	[DIGITAL_MESSAGE](board: Board): void {
...
const SYSEX_RESPONSE: { [key: number]: (board: Board) => void } = {
	/**
	...
	[QUERY_FIRMWARE](board: Board): void {
	...
	[CAPABILITY_RESPONSE](board: Board): void {
		const modes = Object.keys(board.MODES).map((key) => board.MODES[key]);
		let mode, resolution;
		let capability = 0;

		function supportedModes(capability: number): number[] {
			return modes.reduce((accum: number[], mode) => {
	...
	[PIN_STATE_RESPONSE](board: Board): void {
	...
	[ANALOG_MAPPING_RESPONSE](board: Board): void {
		let pin = 0;
		let currentValue;
	...
	[ONEWIRE_SEARCH_REPLY](board: Board): void {
	...
	[ONEWIRE_SEARCH_ALARMS_REPLY](board: Board): void {
		const pin = board.buffer[3];
		const buffer = board.buffer.slice(4, board.buffer.length - 1);
	...
	[ONEWIRE_READ_REPLY](board: Board): void {
	...
	[SERIAL_MESSAGE](board: Board): void {
...
export class Firmata extends Emitter implements Board {
	isReady: boolean;
	MODES: {
		INPUT: number;
		OUTPUT: number;
	...
	STEPPER: {
		...
		STEP_SIZE: { WHOLE: number; HALF: number };
		RUN_STATE: { STOP: number; ACCEL: number; DECEL: number; RUN: number };
	...
	RESOLUTION: { ADC: null | number; DAC: null | number; PWM: null | number };
	HIGH: number;
	LOW: number;
	pins: Pin[];
	ports: any[];
	analogPins: number[];
	version: { major: number; minor: number } = { major: 0, minor: 0 };
	firmware: {};
	buffer: number[];
	...
	constructor(
		...
		this.transport.on("close", (event: any) => {
		...
		this.transport.on("open", (event: any) => {
		...
		this.transport.on("data", (data: number[]) => {
	...
	digitalWrite(pin: number, value: number, enqueue?: number) {
	...
	updateDigitalPort(pin: number, value: number) {
		const port = pin >> 3;
		...
		if (typeof options === "number") {
			...
			if (typeof options === "object" && options !== null) {
				...
				if (typeof options.address !== "undefined") {
					if (!settings[options.address as string]) {
						settings[options.address as string] = {
							stopTX: true,
						};
					}
				}
	...
	i2cWrite(address: number, registerOrData: any, inBytes: number[]) {
		/**
		 * registerOrData:
		 * [... arbitrary bytes]
		 *
	...
	i2cWriteReg(address: number, register: number, byte: number) {
		i2cRequest(this, [
			START_SYSEX,
			I2C_REQUEST,
			address,
	...
	sendOneWireConfig(pin: number, enableParasiticPower: any) {
		writeToTransport(this, [
			START_SYSEX,
			ONEWIRE_DATA,
	...
	sendOneWireSearch(pin: number, callback: () => void) {
	...
	sendOneWireAlarmsSearch(pin: number, callback: () => void) {
		this[SYM_sendOneWireSearch](
	...
	[SYM_sendOneWireSearch](
		...
		event: string | symbol,
		pin: any,
		callback: (arg0: Error | null, arg1?: undefined) => void,
	) {
	...
	sendOneWireWrite(pin: number, device: any, data: any) {
		this[SYM_sendOneWireRequest](
			pin,
	...
	sendOneWireDelay(pin: number, delay: number) {
		this[SYM_sendOneWireRequest](
			pin,
	...
	[SYM_sendOneWireRequest](
		...
		if (device) {
			bytes.splice(...([0, 8].concat(device) as [number, number, ...number[]]));
		}
	...
	reportAnalogPin(pin: number, value: number) {
		/* istanbul ignore else */
		if (value === 0 || value === 1) {
	...
	reportDigitalPin(pin: number, value: number) {
		const port = pin >> 3;
		/* istanbul ignore else */
		if (value === 0 || value === 1) {
	...
	accelStepperConfig(options: any) {
		const {
			deviceNum,
			invertPins,
			motorPin1,
	...
	accelStepperZero(deviceNum: number) {
		writeToTransport(this, [
			START_SYSEX,
			ACCELSTEPPER,
	...
	accelStepperEnable(deviceNum: number, enabled = true) {
		writeToTransport(this, [
			START_SYSEX,
			ACCELSTEPPER,
	...
	accelStepperStop(deviceNum: number) {
		writeToTransport(this, [
			START_SYSEX,
			ACCELSTEPPER,
	...
	accelStepperAcceleration(deviceNum: number, acceleration: number) {
		writeToTransport(this, [
			START_SYSEX,
			ACCELSTEPPER,
	...
	accelStepperSpeed(deviceNum: number, speed: number) {
		writeToTransport(this, [
			START_SYSEX,
			ACCELSTEPPER,
	...
	multiStepperConfig(options: any) {
		writeToTransport(this, [
			START_SYSEX,
			ACCELSTEPPER,
	...
	multiStepperStop(groupNum: number) {
		/* istanbul ignore else */
		if (groupNum < 0 || groupNum > 5) {
			throw new RangeError(
				`Invalid "groupNum": ${groupNum}. Expected "groupNum" between 0-5`,
	...
	serialConfig(
		options: { portId: any; baud: any; rxPin: any; txPin: any } | null,
	) {
		let portId;
		let baud;
		let rxPin;
	...
	serialWrite(portId: number, bytes: number[]) {
	...
	sysexResponse(
		...
		Firmata.SYSEX_RESPONSE[commandByte] = (board: { buffer: string | any[] }) =>
	...
	clearSysexResponse(commandByte: number) {
		/* istanbul ignore else */
		if (Firmata.SYSEX_RESPONSE[commandByte]) {
	...
	static isAcceptablePort(port: { path: string }) {
	...
	static requestPort(callback: (arg0: Error | null, arg1: null) => void) {
		...
		Transport.list()
			.then((ports) => {
				const port = ports.find(
					(port: { path: string }) => Firmata.isAcceptablePort(port) && port,
				);
				if (port) {
					callback(null, port);
				} else {
...
(Firmata as any).Board = Firmata;
(Firmata as any).SYSEX_RESPONSE = SYSEX_RESPONSE;
(Firmata as any).MIDI_RESPONSE = MIDI_RESPONSE;
 ...
 */
export function writeToTransport(board: Firmata, data: number[]): void {
	board.pending++;
	board.transport.write(Buffer.from(data), () => {
		board.pending--;
	});
}
 ...
 */
export function i2cRequest(board: Firmata, bytes: number[]): void {
	...
	if (bytes[1] === I2C_REQUEST) {
		...
		if (bytes[3] & I2C_READ_MASK) {
			// Invert logic to accommodate default = true,
 ...
 */
export function encode32BitSignedInteger(data: number): number[] {
 ...
 */
export function decode32BitSignedInteger(bytes: number[]): number {
	let result =
		(bytes[0] & 0x7f) |
		((bytes[1] & 0x7f) << 7) |
		((bytes[2] & 0x7f) << 14) |
 ...
 * @returns An array of encoded numbers.
 */
export function encodeCustomFloat(input: number): number[] {
 ...
 */
export function decodeCustomFloat(input: number[]): number {
...
if (process.env.IS_TEST_MODE) {
	/* istanbul ignore else */
	let transportBackup: any = null;
	(Firmata as any).test = {
		i2cPeripheralSettings(board: Firmata) {
		...
		set transport(value: any) {
			transportBackup = Transport;
			Transport = value;
		},
		restoreTransport() {
			Transport = transportBackup;
		},
		encode32BitSignedInteger,
		decode32BitSignedInteger,
		encodeCustomFloat,
 ...
 */
const bindTransport = (transport: any): typeof Firmata => {
	Transport = transport;
	return Firmata;
};

(bindTransport as any).Firmata = Firmata;
